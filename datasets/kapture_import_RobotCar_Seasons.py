...
"""
This script imports a RobotCar dataset to the kapture format. Based off of the original script from
https://github.com/naver/kapture/blob/master/tools/kapture_import_RobotCar_Seasons.py

Key differences:

- Separates mapping (overcast reference), train (queries from different conditions with ground truth pose
  available) and query (to be submitted for evaluation on https://www.visuallocalization.net/submission/).

  The original file combines train and mapping sets together within a submodel.

- This file provides two Kaptures, one where the mapping dataset is based off of the combined SfM model
  provided in the original benchmark and merging is performed across the submodels (i.e. each condition
  has its own single kapture directory) and one where each submodel is kept separately.

  The full combined model is larger scale and much more relevant for assessing image retrieval methods as
  opposed to structure-based pose refinement methods.

- See https://github.com/naver/kapture/blob/master/doc/datasets.adoc for more information.
"""
...
def import_robotcar_seasons(robotcar_path: str,
    ...
    if rig_collapse:
        ...
        for kdata_training in kapture_imported_training.values():
            kapture.rigs_recover_inplace(kdata_training.trajectories, rigs, 'rear')
    ...
    for loc_id, kdata_training in kapture_imported_training.items():
        ...
        kapture_training_dir = path.join(kapture_path, f"{loc_id:02d}", "training")
        ...
        mapping_images = [f for _, _, f in kapture.flatten(kdata_training.records_camera)]
        import_record_data_from_dir_auto(robotcar_image_path, kapture_training_dir,
                                         mapping_images, images_import_method)
...
def import_robotcar_seasons_command_line() -> None:
    ...
    parser.add_argument('--image_transfer', type=TransferAction, default=TransferAction.link_relative,
    ...
    import_robotcar_seasons(args.input, args.output, args.force, args.image_transfer,
                            args.skip_reconstruction, args.rig_collapse,
